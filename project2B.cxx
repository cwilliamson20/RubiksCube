...
RenderManager::RenderManager()
{
  ...
  glm::vec4 lightcoeff(0.3, 0.7, 0.5, 50.5); // Lighting coeff, Ka, Kd, Ks, alpha
...
RenderManager::SetUpWindowAndShaders()
{
  ...
  window = glfwCreateWindow(1000, 1000, "CIS 441", NULL, NULL);
  if (!window) {
...
{
  // make the head at position 3, 1, 0
    glm::mat4 translate = TranslateMatrix(3, 1, 0);
  rm.SetColor(0.8, 0.4, 0.0);
  rm.Render(RenderManager::SPHERE, translate);

   glm::mat4 leftEyeTranslate = TranslateMatrix(-0.15, 0.25, 1);
   glm::mat4 rotateInFromLeft = RotateMatrix(15, 0, 1, 0);
   SetUpEyeball(modelSoFar*translate*leftEyeTranslate, rm);

   glm::mat4 rightEyeTranslate = TranslateMatrix(0.15, 0.25, 1);
   glm::mat4 rotateInFromRight = RotateMatrix(-15, 0, 1, 0);
   SetUpEyeball(modelSoFar*translate*rightEyeTranslate, rm);

   // make the snout below the eyes
   rm.SetColor(1, .7, .4);
   glm::mat4 snout_translate = TranslateMatrix(0, -.3, 1);
   glm::mat4 snout_scale = ScaleMatrix(0.5, 0.3, 0.3);
   rm.Render(RenderManager::SPHERE, translate*snout_translate*snout_scale);

   // make the tongue coming out of the snout
   rm.SetColor(100, 0, 0);
   glm::mat4 tongue_translate = TranslateMatrix(0, -.3, 1.2);
   glm::mat4 tongue_scale = ScaleMatrix(0.2, 0.05, 0.2);
   rm.Render(RenderManager::SPHERE, translate*tongue_translate*tongue_scale);

   // make the neck, angled between the neck and where the body will be
   rm.SetColor(.8, .4, 0);
   glm::mat4 neck_scale = ScaleMatrix(.25, .25, 2);
   glm::mat4 neck_translate = TranslateMatrix(0, -1, -1.5);
   glm::mat4 neck_rotation = RotateMatrix(315, 1, 0, 0);
   rm.Render(RenderManager::CYLINDER, translate*neck_translate*neck_rotation*neck_scale);

   // make the ears
   // left ear
   glm::mat4 ear_scale = ScaleMatrix(.3, .6, .05);
   glm::mat4 ear_translate = TranslateMatrix(.7, .5, .5);
   glm::mat4 ear_rotation1 = RotateMatrix(320, 0, 0, 1);
   glm::mat4 ear_rotation2 = RotateMatrix(350, 0, 1, 0);
   rm.Render(RenderManager::SPHERE, translate*ear_translate*ear_rotation1*ear_rotation2*ear_scale);

   // right ear
   ear_translate = TranslateMatrix(-.7, .5, .5);
   ear_rotation1 = RotateMatrix(40, 0, 0, 1);
   ear_rotation2 = RotateMatrix(10, 0, 1, 0);
   rm.Render(RenderManager::SPHERE, translate*ear_translate*ear_rotation1*ear_rotation2*ear_scale);
}
...
SetUpDog(int counter, RenderManager &rm)
{
  ...
  // set up the body oval
  glm::mat4 body_scale = ScaleMatrix(1, 1, 2);
  glm::mat4 body_translate = TranslateMatrix(3, 0, -1.5);
  rm.Render(RenderManager::SPHERE, body_scale*body_translate);

  // set up tail (this is the thing that animates)
  glm::mat4 tail_scale = ScaleMatrix(.1, .1, .8);
  glm::mat4 tail_translate = TranslateMatrix(3, .5, -4.8);
  glm::mat4 tail_rotation = RotateMatrix(40, 1, 0, 0);
  // counter goes from 0 to 310
  // want the tail to move 15 degrees back and forth depending on what camera is
  // tail_counter goes from 0 to 60
  int tail_counter = counter;
  while (tail_counter > 60) {
    tail_counter -= 60;
  }
  // want tail to wag back and forth depending on the value of counter
  int tail_pos = 0;
  if (tail_counter < 15) {
    tail_pos = tail_counter;
  } else if (tail_counter < 30) {
    tail_pos = 15 - (tail_counter % 15);
  } else if (tail_counter == 30) {
    tail_pos = 0;
  } else if (tail_counter < 45) {
    tail_pos = 365 - (tail_counter - 30);
  } else {
    tail_pos = 350 + (tail_counter - 45);
  }
  glm::mat4 tail_animation = RotateMatrix(tail_pos, 0, 1, 0);
  rm.Render(RenderManager::SPHERE, tail_translate*tail_rotation*tail_animation*tail_scale);

  // set up legs
  glm::mat4 leg_scale = ScaleMatrix(.2, .2, .8);
  glm::mat4 leg_rotation = RotateMatrix(90, 1, 0, 0);

  // front right
  glm::mat4 leg_translate = TranslateMatrix(2.5, -.5, -2.1);
  rm.Render(RenderManager::CYLINDER, leg_translate*leg_rotation*leg_scale);

  // front left
  leg_translate = TranslateMatrix(3.5, -.5, -2.1);
  rm.Render(RenderManager::CYLINDER, leg_translate*leg_rotation*leg_scale);

  // back right
  leg_translate = TranslateMatrix(2.5, -.5, -3.9);
  rm.Render(RenderManager::CYLINDER, leg_translate*leg_rotation*leg_scale);

  // back left
  leg_translate = TranslateMatrix(3.5, -.5, -3.9);
  rm.Render(RenderManager::CYLINDER, leg_translate*leg_rotation*leg_scale);


  // set up the feet
  rm.SetColor(0, 0, 0);
  glm::mat4 foot_scale = ScaleMatrix(.3, .2, .35);

  // front right
  glm::mat4 foot_translate = TranslateMatrix(2.5, -1.4, -2);
  rm.Render(RenderManager::SPHERE, foot_translate*foot_scale);

  // front left
  foot_translate = TranslateMatrix(3.5, -1.4, -2);
  rm.Render(RenderManager::SPHERE, foot_translate*foot_scale);

  // back right
  foot_translate = TranslateMatrix(2.5, -1.4, -3.8);
  rm.Render(RenderManager::SPHERE, foot_translate*foot_scale);

  // back left
  foot_translate = TranslateMatrix(3.5, -1.4, -3.8);
  rm.Render(RenderManager::SPHERE, foot_translate*foot_scale);
}
...
const char *GetVertexShader()
{
  ...
  "float norm = sqrt((viewdir[0]*viewdir[0])+(viewdir[1]*viewdir[1])+(viewdir[2]*viewdir[2]));\n"
  "for (int x = 0; x < 3; x++) {\n"
      "viewdir[x] = viewdir[x] / norm;\n"
  "}\n"
  ...
  "for (int x = 0; x < 3; x++) {\n"
      "r[x] = (2 * LdotN * vertex_normal[x]) - lightdir[x]; \n"
  "}\n"
  "norm = sqrt((r[0]*r[0])+(r[1]*r[1])+(r[2]*r[2]));\n"
  "for (int x = 0; x < 3; x++) {\n"
      "r[x] = r[x] / norm;\n"
  "}\n"
  "float RdotV = (r[0]*viewdir[0])+(r[1]*viewdir[1])+(r[2]*viewdir[2]);\n"
  "RdotV = max(0.0, RdotV);\n"
  "float specular = abs(lightcoeff[2] * (pow(max(0.0, RdotV), lightcoeff[3])));\n"
  "shading_amount = lightcoeff[0] + (lightcoeff[1] * diffuse) + specular;\n" 
  "}\n";
  //  strcpy(vertexShader, 
  //          "#version 400\n"
  //          "layout (location = 0) in vec3 vertex_position;\n"
